...
export async function merge(projectPath, branch, mergeBranch) {
  const repo = await Git.Repository.open(`${projectPath}/.git`);
  var mergeOptions = new Git.MergeOptions();
  mergeOptions.fileFavor = Git.Merge.FILE_FAVOR.THEIRS;
  return await repo.mergeBranches(branch,
    mergeBranch,
...
export async function reset_hard(projectPath) {
  const repo = await Git.Repository.open(`${projectPath}/.git`);
  return await Git.Reset.reset(repo,
    await repo.getReferenceCommit('master'),
    Git.Reset.TYPE.HARD);
}
